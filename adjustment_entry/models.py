...
class AdjustmentEntry(models.Model):
    """Adjustment Journal Entry model for ERP accounting system"""
    ...
    adjustment_type = models.CharField(max_length=20, choices=ADJUSTMENT_TYPE_CHOICES, default='others')
    
    # Status and Workflow
    status = models.CharField(max_length=20, choices=STATUS_CHOICES, default='draft')
    posted_at = models.DateTimeField(blank=True, null=True)
    posted_by = models.ForeignKey(User, on_delete=models.PROTECT, related_name='adjustment_entries_posted', blank=True, null=True)
    
    # Audit Fields
    created_by = models.ForeignKey(User, on_delete=models.PROTECT, related_name='adjustment_entries_created')
    created_at = models.DateTimeField(auto_now_add=True)
    updated_by = models.ForeignKey(User, on_delete=models.PROTECT, related_name='adjustment_entries_updated', blank=True, null=True)
    ...
    def __str__(self):
        return f"AJE-{self.voucher_number} - {self.get_adjustment_type_display()} - {self.narration[:50]}"
    
    def save(self, *args, **kwargs):
        if not self.voucher_number:
            # Generate voucher number
            year = timezone.now().year
            last_entry = AdjustmentEntry.objects.filter(
                voucher_number__startswith=f'AJE-{year}'
            ...
            self.voucher_number = f'AJE-{year}-{new_number:04d}'
    ...
    def adjustment_type_display(self):
        """Get formatted adjustment type display"""
        return self.get_adjustment_type_display()


class AdjustmentEntryDetail(models.Model):
    """Individual debit/credit entries within an adjustment entry"""
    
    adjustment_entry = models.ForeignKey(AdjustmentEntry, on_delete=models.CASCADE, related_name='entries')
    ...
    def __str__(self):
        ...
        return f"{self.adjustment_entry.voucher_number} - {self.account.name} - {amount}"
...
class AdjustmentEntryAudit(models.Model):
    ...
    adjustment_entry = models.ForeignKey(AdjustmentEntry, on_delete=models.CASCADE, related_name='audit_trail')
    ...
    def __str__(self):
        return f"{self.adjustment_entry.voucher_number} - {self.action} by {self.user.get_full_name()}"
