...
function updateHiddenInputs() {
    ...
    $('input[name^="selected_"]').remove();
    
    // Get form element
    const form = $('form');
    if (!form.length) return;
    
    // Add hidden inputs for selected invoices
    $('.invoice-checkbox:checked').each(function(index) {
        const invoiceNumber = $(this).data('invoice');
        const amount = $(this).data('amount');
        
        // Create hidden input for invoice number
        $('<input>', {
            type: 'hidden',
            name: 'selected_invoices_' + index,
            value: invoiceNumber
        }).appendTo(form);
        ...
        $('<input>', {
            type: 'hidden',
            name: 'selected_amounts_' + index,
            value: amount
        }).appendTo(form);
    });
    
    // Add hidden inputs for selected bills
    $('.bill-checkbox:checked').each(function(index) {
        const billNumber = $(this).data('bill');
        const amount = $(this).data('amount');
        
        // Create hidden input for bill number
        $('<input>', {
            type: 'hidden',
            name: 'selected_bills_' + index,
            value: billNumber
        }).appendTo(form);
        ...
        $('<input>', {
            type: 'hidden',
            name: 'selected_bill_amounts_' + index,
            value: amount
        }).appendTo(form);
    });
}
