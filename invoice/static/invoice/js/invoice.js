...
function generateVendorOptions() {
    ...
    let options = '<option value="">Select vendor or payment source</option>';
    ...
    // Add vendors (without optgroup label)
    if (availableVendors && availableVendors.length > 0) {
        const vendors = availableVendors.filter(item => item && item.type === 'vendor');
    ...
    // Add payment sources (without optgroup label)
    if (availablePaymentSources && availablePaymentSources.length > 0) {
        availablePaymentSources.forEach((source, index) => {
            if (source && source.id && source.name) {
                const displayName = source.description ? `${source.name} - ${source.description}` : source.name;
                options += `<option value="payment_source_${source.id}" data-type="payment_source">${displayName}</option>`;
            }
        });
    }
    
    // If no options were added (only default option), add a message
    if (options === '<option value="">Select vendor or payment source</option>') {
    ...
    rows.forEach(row => {
        ...
        if (vendorSelect) {
            vendorSelect.innerHTML = `
                <option value="">Select a vendor or payment source</option>
...
function refreshVendorDropdowns() {
    ...
    rows.forEach(row => {
        ...
        if (vendorSelect) {
            vendorSelect.innerHTML = `
                <option value="">Select vendor or payment source</option>
                ${generateVendorOptions()}
            `;
        }
    });
